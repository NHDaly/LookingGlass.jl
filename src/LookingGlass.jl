...
"""
    module_globals_names(m; consts=true, nonconsts=true) -> Vector{Symbol}
Return a list of the names of all global variables in Module `m`.

Defaults to returning all globals, but you can toggle only const-globals or nonconst-globals
via the keyword arguments.
"""
module_globals_names(m::Module; consts=true, nonconsts=true) =
    [n for n in names(m, all=true)
        if module_name_isglobal(m, n; consts=consts, nonconsts=nonconsts)]
function module_name_isglobal(m::Module, n::Symbol; consts, nonconsts)
    try
        String(n)[1] != '#' &&
        !isa(Core.eval(m, n), Union{DataType, Function, Module}) &&
        (consts && _isconst_global(m, n)) || (nonconsts && !_isconst_global(m, n))
    catch
        false
    end
end
...
"""
    module_recursive_globals_names(m; consts=true, nonconsts=true) -> Dict(submodule => name)
...
"""
module_recursive_globals_names(m::Module; consts=true, nonconsts=true) =
    merge!(
        Dict(m => module_globals_names(m, consts=consts, nonconsts=nonconsts)),
        Dict(
            sm => names
            for sm in module_submodules(m, recursive=true)
            for names in (module_globals_names(sm; consts=consts, nonconsts=nonconsts),)
            if !isempty(names)
        ))
